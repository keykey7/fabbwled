    ...
    public SectionNode getBody(PlayerDto current) {
        return SectionNode.root()
                ...
                .activeIf(current.blessings().contains(BlessingEnum.ALVIR_AND_VALMIR), a -> a
                        ...
                        .clickableTurnTo(439)
                    .activeElse(b -> b.activeIf(current.))
                        Roll one die if you ship is a barque, two dice if it is abrigantine, or three dice if a galleon. Add 1 to the roll if you have a good crew; add 2 if the crew is excellent.Score 1-3 Ship sinks turn to 485
                );
    }
}
 */
